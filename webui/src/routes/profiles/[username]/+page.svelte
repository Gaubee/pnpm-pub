<script lang="ts">
  ...
  import OtpButton from "$lib/components/otp-button.svelte";
...
<svelte:head>
  <title
    >{profile
      ? $_("profile.title", { values: { username: profile.username } })
      : $_("profile.noProfile")}</title
  >
</svelte:head>
  ...
  <div class="mx-auto flex max-w-3xl flex-col gap-5 p-6">
    ...
    <section class="rounded-lg border border-border bg-card p-4">
      <div class="flex items-center justify-between gap-3">
        <div
          class="flex min-w-0 items-center gap-2 text-xs font-medium uppercase text-muted-foreground"
        >
          <IconKey />
          {$_("profile.npmToken")}
          {#if autoRenewOn}
          ...
          <ButtonGroup.Root>
            ...
            {#if token}
              <Button
                ...
                aria-label={showToken
                  ? $_("addProfile.hidePassword")
                  : $_("addProfile.showPassword")}
                title={showToken ? $_("addProfile.hidePassword") : $_("addProfile.showPassword")}
              >
                {#if showToken}<IconEyeOff class="h-3.5 w-3.5" />{:else}<IconEye
                    class="h-3.5 w-3.5"
                  />{/if}
              </Button>
              <Button
                ...
                size="icon-sm"
                onclick={copyToken}
                aria-label={$_("profile.copyToken")}
                title={$_("profile.copyToken")}
              >
                {#if copied}<IconCheck class="h-3.5 w-3.5 text-success" />{:else}<IconCopy
                    class="h-3.5 w-3.5"
                  />{/if}
              </Button>
            ...
            {:else}
              <Button
                ...
                size="icon-sm"
                onclick={loadToken}
                aria-label={$_("profile.revealToken")}
                title={$_("profile.revealToken")}
              >
            ...
            <div class="flex items-start justify-between gap-2">
              ...
              {#if !renewForced}
                <Button
                  variant="ghost"
                  size="icon"
                  class="-mr-1.5 -mt-1 h-6 w-6 shrink-0"
                  onclick={() => (renewOpen = false)}
                  disabled={reauthBusy}
                  aria-label={$_("profile.reauthCancel")}
                >
              ...
              <div class="space-y-1.5">
                ...
                <div class="relative">
                  ...
                  <Button
                    variant="ghost"
                    size="icon-sm"
                    class="absolute right-0.5 top-0.5 h-6 w-6 text-muted-foreground"
                    onclick={() => (reauthShowPassword = !reauthShowPassword)}
                    disabled={reauthBusy}
                    aria-label={reauthShowPassword
                      ? $_("addProfile.hidePassword")
                      : $_("addProfile.showPassword")}
                    title={reauthShowPassword
                      ? $_("addProfile.hidePassword")
                      : $_("addProfile.showPassword")}
                    tabindex={-1}
                  >
                    {#if reauthShowPassword}<IconEyeOff class="h-3.5 w-3.5" />{:else}<IconEye
                        class="h-3.5 w-3.5"
                      />{/if}
                  </Button>
                </div>
            ...
            {:else}
              <div class="space-y-1.5">
                ...
                <p class="text-[11px] text-muted-foreground">
                  {$_("profile.reauthManualTokenHint")}
                </p>
              </div>
            {/if}
              ...
              {#if detail.email}
                <div class="flex items-center gap-2 text-xs">
                  ...
                  <a
                    ...
                    class="truncate text-foreground transition-colors hover:text-brand hover:underline"
                    >{detail.email}</a
                  >
                  {#if detail.emailVerified === false}
                    <span class="shrink-0 text-[10px] text-warning"
                      >({$_("profile.emailUnverified")})</span
                    >
                  {/if}
                </div>
              {/if}
              ...
              {#if detail.github || detail.twitter || detail.homepage}
                <div class="space-y-1 pt-0.5 flex flex-col gap-2">
                  <span
                    ...
                    >{$_("profile.linkedAccounts")}</span
                  >
                  {#if detail.github}
                    <div class="flex items-center gap-2 text-xs">
                      <svg
                        class="block h-3.5 w-3.5 shrink-0 self-center text-muted-foreground"
                        viewBox="0 0 24 24"
                        fill="currentColor"
                        aria-hidden="true"
                        ><path
