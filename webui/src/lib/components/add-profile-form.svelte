...
{:else}
  ...
  >
    ...
    <div class="flex flex-col gap-2 mt-4">
      ...
      <InputGroup class="border-black/50">
        <InputGroupInput
          id="ap-t"
          bind:value={totpRaw}
      ...
      {:else if totpSecret}
        <p class="text-[11px] {totpHint ? 'text-warning' : 'text-muted-foreground'}">
          {$_("addProfile.recognizedAs", { values: { secret: totpSecret } })}{#if parsedTotp?.label}
            ({parsedTotp.label}){/if}
        </p>
        {#if totpHint}
        ...
        <InputGroup class="border-black/50">
          ...
          <InputGroupAddon align="inline-end">
            <button
              type="button"
              tabindex="-1"
              title={showPassword ? $_("addProfile.hidePassword") : $_("addProfile.showPassword")}
              class="flex h-5 w-5 items-center justify-center rounded text-muted-foreground transition-colors hover:text-foreground"
            ...
            >
              {#if showPassword}<IconEyeOff class="h-4 w-4" />{:else}<IconEye
                  class="h-4 w-4"
                />{/if}
            </button>
          </InputGroupAddon>
    ...
    {#if phase === "manual"}
      <div
        class="mt-4 flex items-start gap-2 rounded-md border border-warning/40 bg-warning/10 px-3 py-2 text-xs"
      >
    ...
    {#if error && phase !== "manual"}
      ...
      >
        ...
        <div class="wrap-break-word">{error}</div>
      </div>
    {/if}
