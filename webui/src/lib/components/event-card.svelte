<script lang="ts">
	...
	const statusVariant = $derived(
		({
			...
			expired: 'warning',
			rejected: 'secondary',
		})[event.status] as 'brand' | 'success' | 'destructive' | 'warning' | 'secondary',
	);
	...
	// Both 'publish' and 'create-placeholder' carry a target; narrow to that shape.
	const publishTarget = $derived(
		event.payload?.kind === 'publish'
			? event.payload.data
			: event.payload?.kind === 'create-placeholder'
				? ({ target: { name: event.payload.data.name, version: '0.0.0', previousVersion: undefined, description: undefined, path: event.payload.data.path } })
				: null,
	);
...
<Card class="transition-shadow {isPending ? 'ring-2 ring-brand/40 shadow-md' : ''}">
	...
	<CardContent class="space-y-3">
		...
		{#if isPending}
			<div class="flex items-center gap-2 pt-1">
				<Button variant="brand" size="sm" class="flex-1" onclick={() => actions.confirm(event.id)}>
					...
					{#if event.kind === 'setup-oidc'}Confirm Setup OIDC{:else}Confirm Publish{/if}
				</Button>
		...
		{:else if isExpired}
			<!-- Chapter 6.2.4: expired token surfaces a renew prompt. -->
			<Button variant="outline" size="sm" class="w-full" onclick={() => (window.location.href = '/renew')}>
				Token expired — renew now
			</Button>
		{/if}
	</CardContent>
</Card>
