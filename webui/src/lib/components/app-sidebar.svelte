...
>
  ...
  <nav class="no-drag flex flex-1 flex-col gap-1 p-3">
    {#each nav as item (item.href)}
      ...
      {@const active =
        pathname === item.href || (item.href === "/active-events" && pathname === "/")}
  ...
  <div class={cn("no-drag relative border-t border-sidebar-border", collapsed ? "py-3" : "p-3")}>
    <button
      type="button"
    ...
    >
      <Avatar>
        {#if $activeProfile?.username}
          ...
          <AvatarFallback
            >{$activeProfile ? initials($activeProfile.username) : "??"}</AvatarFallback
          >
        {:else}
        ...
        {#each $daemon.profiles as p (p.username)}
          ...
          >
            <Avatar class="h-6 w-6">
              <AvatarImage src={avatarUrlFor(p.username)} alt={p.username} />
