...
>
    ...
    <nav class="no-drag flex flex-1 flex-col gap-1 p-3">
        {#each nav as item (item.href)}
            ...
            {@const active =
                pathname === item.href ||
                (item.href === "/active-events" && pathname === "/")}
            {@const label = $_(item.labelKey)}
            {@const showBadge =
                item.href === "/active-events" && $pendingEvents.length > 0}
            <a
                href={item.href}
                class={cn(
            ...
            >
                ...
                {#if showBadge && !collapsed}
                    <Badge variant="brand" class="h-5 px-1.5 text-[10px]"
                        >{$pendingEvents.length}</Badge
                    >
                {:else if showBadge}
                    <span
                        class="absolute ml-5 mt-[-1.25rem] h-2 w-2 rounded-full bg-brand"
                    ></span>
                {/if}
            </a>
        {/each}
        ...
        {#if pinned.length > 0 && !collapsed}
            <div
                class="mt-4 px-3 pb-1 text-[10px] font-semibold uppercase tracking-wider text-muted-foreground/70"
            >
                {$_("sidebar.pinned")}
            </div>
            {#each pinned as ws (ws.path)}
                ...
                >
                    ...
                    <span class="truncate"
                        >{ws.path.split("/").pop() ?? ws.path}</span
                    >
                </a>
            {/each}
        {/if}
    ...
    <div
        class={cn(
            "no-drag relative border-t border-sidebar-border",
            collapsed ? "py-3" : "p-3",
        )}
    >
        <button
            ...
            title={collapsed
                ? ($activeProfile?.username ?? $_("sidebar.noProfile"))
                : undefined}
        >
            <Avatar>
                {#if $activeProfile?.username}
                    <AvatarImage
                        src={avatarUrlFor($activeProfile.username)}
                        alt={$activeProfile.username}
                    />
                    <AvatarFallback
                        >{$activeProfile
                            ? initials($activeProfile.username)
                            : "??"}</AvatarFallback
                    >
                {:else}
            ...
            {#if !collapsed}
                <div class="min-w-0 flex-1">
                    ...
                    <div class="truncate text-[11px] text-muted-foreground">
                        {$_("sidebar.profileCount", {
                            values: { count: $daemon.profiles.length },
                        })}
                    </div>
                </div>
                ...
                {#each $daemon.profiles as p (p.username)}
                    <button
                        ...
                        onclick={() => {
                            ...
                            menuOpen = false;
                            goto(
                                `/profiles/${encodeURIComponent(p.username)}${location.hash}`,
                            );
                        }}
                    ...
                    >
                        <Avatar class="h-6 w-6">
                            <AvatarFallback class="text-[10px]"
                                >{initials(p.username)}</AvatarFallback
                            >
                        </Avatar>
