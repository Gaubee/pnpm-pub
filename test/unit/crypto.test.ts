...
describe("PBKDF2 key derivation", () => {
  ...
  it("is deterministic for the same salt + password", () => {
    ...
    expect(deriveKey(Buffer.from("pw", "utf8"), salt)).toEqual(
      deriveKey(Buffer.from("pw", "utf8"), salt),
    );
  });

  it("differs across passwords", () => {
    const salt = Buffer.alloc(16, 9);
    expect(deriveKey(Buffer.from("pw1", "utf8"), salt)).not.toEqual(
      deriveKey(Buffer.from("pw2", "utf8"), salt),
    );
  });
