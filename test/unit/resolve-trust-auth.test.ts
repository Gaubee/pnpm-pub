...
function isProfilesFrame(value: unknown): value is Extract<WsServerMessage, { type: 'profiles' }> {
  return (
    typeof value === 'object' &&
    value !== null &&
    (value as { type?: string }).type === 'profiles'
  );
}
...
describe('resolveTrustAuth three-state liveness probe', () => {
  ...
  beforeEach(async () => {
    ...
    mocks.readProfileDetailMock.mockResolvedValue({
      name: 'alice', fullname: 'Alice', email: null, emailVerified: null,
      github: null, twitter: null, homepage: null, tfaEnabled: null, createdAt: null,
    });
  ...
  it('Scenario: Given an expired token, When probing, Then it returns 401 needsReauth and flips authStatus', async () => {
    mocks.verifyCredentialsMock.mockResolvedValueOnce({
      ok: true,
      status: 401,
      check: { authValid: false, requires2FA: false, otpValid: null, message: 'auth invalid (E401)' },
    });
