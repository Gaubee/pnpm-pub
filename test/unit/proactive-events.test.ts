...
describe('Feature: WebUI-created proactive events', () => {
  it('Scenario: Given a setup-oidc event, When confirmed, Then it executes from the pending wall', async () => {
    ...
    const workflow = await fsp.readFile(path.join(packageDir, '.github/workflows/publish.yml'), 'utf8');
  ...
  it('Scenario: Given publish args with --access value, When confirmed, Then the command access reaches publishPackage', async () => {
    ...
    await fsp.mkdir(packageDir, { recursive: true });
    await fsp.writeFile(path.join(packageDir, 'package.json'), '{"name":"@scope/pkg","version":"1.0.0"}', 'utf8');
    const exit = vi.fn();
    const log = vi.fn();
  ...
  it('Scenario: Given publish args with --access=value, When confirmed, Then the command access reaches publishPackage', async () => {
    ...
    expect(publishPackageMock).toHaveBeenCalledWith(expect.objectContaining({ access: 'restricted' }));
    expect(exit).toHaveBeenCalledWith(0);
  });
  ...
  it('Scenario: Given publish args with --report-summary, When publish succeeds, Then the pnpm summary artifact is written', async () => {
    ...
    const summaryText = await fsp.readFile(path.join(packageDir, 'pnpm-publish-summary.json'), 'utf8');
  ...
  it('Scenario: Given a publish tarball argument, When confirmed, Then the tarball is the publish source', async () => {
    ...
    await fsp.writeFile(`${tarballPath}.package.json`, JSON.stringify({ name: 'requested-pkg', version: '1.2.3' }), 'utf8');
    const exit = vi.fn();
    const log = vi.fn();
  ...
  it('Scenario: Given a config namespace publish option, When dry-run is confirmed, Then it warns and preserves source resolution', async () => {
    ...
    await fsp.mkdir(packageDir, { recursive: true });
    await fsp.writeFile(path.join(packageDir, 'package.json'), JSON.stringify({ name: 'config-option-pkg', version: '1.0.0' }), 'utf8');
    const exit = vi.fn();
    const stdout: string[] = [];
    const stderr: string[] = [];
  ...
  it('Scenario: Given a config namespace token before a directory, When intercepted, Then the directory remains the publish source', async () => {
    ...
    await fsp.writeFile(path.join(packageDir, 'package.json'), JSON.stringify({ name: 'config-root-pkg', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(path.join(sourceDir, 'package.json'), JSON.stringify({ name: 'config-source-pkg', version: '1.0.0' }), 'utf8');
    const exit = vi.fn();
    const stdout: string[] = [];
    const stderr: string[] = [];
  ...
  it('Scenario: Given recursive dry-run publish args, When confirmed, Then workspace packages are packed without registry writes', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9' }), 'utf8');
    await fsp.writeFile(path.join(packageDir, 'package.json'), JSON.stringify({ name: 'requested-pkg', version: '1.2.3' }), 'utf8');
    await fsp.writeFile(path.join(privateDir, 'package.json'), JSON.stringify({ name: 'private-pkg', version: '0.0.1', private: true }), 'utf8');
    const exit = vi.fn();
    const stdout: string[] = [];
    const stderr: string[] = [];
    ...
    const event = store.getEvent(pending.id);
    expect(event?.status).toBe('success');
    expect(event?.result).toBe('Recursive dry run complete; 2 packages packed; no registry write performed.');
  ...
  it('Scenario: Given recursive dry-run publish with a positional directory, When confirmed, Then the current workspace remains the source', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'pnpm-workspace.yaml'), 'packages:\n  - packages/*\n', 'utf8');
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9', private: true }), 'utf8');
    await fsp.writeFile(path.join(packageDir, 'package.json'), JSON.stringify({ name: 'recursive-positional-pkg', version: '1.2.3' }), 'utf8');
    await fsp.writeFile(path.join(positionalDir, 'package.json'), JSON.stringify({ name: 'recursive-positional-other', version: '9.9.9' }), 'utf8');
    const exit = vi.fn();
    const stdout: string[] = [];
  ...
  it('Scenario: Given recursive dry-run publish with --report-summary, When confirmed, Then the workspace summary artifact is written', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'pnpm-workspace.yaml'), 'packages:\n  - packages/*\n', 'utf8');
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9', private: true }), 'utf8');
    await fsp.writeFile(path.join(packageADir, 'package.json'), JSON.stringify({ name: 'summary-a', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(path.join(packageBDir, 'package.json'), JSON.stringify({ name: 'summary-b', version: '2.0.0' }), 'utf8');
    const exit = vi.fn();
    const stdout: string[] = [];
    ...
    await scheduler.intercept(
      {
        command: 'publish',
        cwd: workspaceRoot,
        args: ['--recursive', '--dry-run', '--report-summary', '--filter', 'summary-*', '--no-git-checks'],
      },
      {
        log: (stream, data) => {
          if (stream === 'stdout') stdout.push(data);
    ...
    const summaryText = await fsp.readFile(path.join(workspaceRoot, 'pnpm-publish-summary.json'), 'utf8');
    const summary: unknown = JSON.parse(summaryText);
  ...
  it('Scenario: Given recursive dry-run publish with tarball facts, When confirmed, Then stderr emits npm notice details', async () => {
    ...
    expect(noticeText).toContain('npm notice Publishing to http://package-registry.test/ with tag beta and public access (dry-run)\n');
  ...
  it('Scenario: Given recursive dry-run publish with a path filter, When confirmed, Then only matching packages are packed', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9' }), 'utf8');
    await fsp.writeFile(path.join(packageDir, 'package.json'), JSON.stringify({ name: 'requested-pkg', version: '1.2.3' }), 'utf8');
    await fsp.writeFile(path.join(otherDir, 'package.json'), JSON.stringify({ name: 'other-pkg', version: '2.0.0' }), 'utf8');
    const exit = vi.fn();
    const log = vi.fn();
    ...
    const event = store.getEvent(pending.id);
    expect(event?.status).toBe('success');
    expect(event?.result).toBe('Recursive dry run complete; 1 package packed; no registry write performed.');
    expect(packPackageMock).toHaveBeenCalledTimes(1);
    expect(packPackageMock).toHaveBeenCalledWith(packageDir);
  ...
  it('Scenario: Given recursive dry-run publish from a package with current-project filter, When confirmed, Then only the current package is packed', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'pnpm-workspace.yaml'), 'packages:\n  - packages/*\n', 'utf8');
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9', private: true }), 'utf8');
    await fsp.writeFile(path.join(packageADir, 'package.json'), JSON.stringify({ name: 'dot-a', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(path.join(packageBDir, 'package.json'), JSON.stringify({ name: 'dot-b', version: '1.0.0' }), 'utf8');
    const exit = vi.fn();
    const stdout: string[] = [];
  ...
  it('Scenario: Given recursive dry-run publish with a brace directory filter, When confirmed, Then matching package directories are packed', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'pnpm-workspace.yaml'), 'packages:\n  - packages/*\n', 'utf8');
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9', private: true }), 'utf8');
    await fsp.writeFile(path.join(packageDir, 'package.json'), JSON.stringify({ name: 'brace-pkg', version: '1.2.3' }), 'utf8');
    await fsp.writeFile(path.join(otherDir, 'package.json'), JSON.stringify({ name: 'other-pkg', version: '2.0.0' }), 'utf8');
    const exit = vi.fn();
    const stdout: string[] = [];
  ...
  it('Scenario: Given recursive dry-run publish with a combined package and directory filter, When confirmed, Then only the intersection is packed', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'pnpm-workspace.yaml'), 'packages:\n  - packages/*\n  - other/*\n', 'utf8');
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9', private: true }), 'utf8');
    await fsp.writeFile(path.join(packageDir, 'package.json'), JSON.stringify({ name: 'combo-pkg', version: '1.2.3' }), 'utf8');
    await fsp.writeFile(path.join(siblingDir, 'package.json'), JSON.stringify({ name: 'sibling-pkg', version: '2.0.0' }), 'utf8');
    await fsp.writeFile(path.join(outsideDir, 'package.json'), JSON.stringify({ name: 'combo-pkg', version: '3.0.0' }), 'utf8');
    const exit = vi.fn();
    const stdout: string[] = [];
  ...
  it('Scenario: Given recursive dry-run publish with a combined package directory graph filter, When confirmed, Then dependencies are packed before the intersected package', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'pnpm-workspace.yaml'), 'packages:\n  - packages/*\n', 'utf8');
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9', private: true }), 'utf8');
    await fsp.writeFile(path.join(depDir, 'package.json'), JSON.stringify({ name: 'combo-dep', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(
      path.join(appDir, 'package.json'),
      JSON.stringify({ name: 'combo-app', version: '1.0.0', dependencies: { 'combo-dep': 'workspace:*' } }),
      'utf8',
    );
    await fsp.writeFile(path.join(otherDir, 'package.json'), JSON.stringify({ name: 'combo-other', version: '1.0.0' }), 'utf8');
    const exit = vi.fn();
    const stdout: string[] = [];
    ...
    await scheduler.intercept(
      {
        command: 'publish',
        cwd: workspaceRoot,
        args: ['--recursive', '--dry-run', '--filter', 'combo-app{packages/app}...', '--no-git-checks'],
      },
      {
        log: (stream, data) => {
          if (stream === 'stdout') stdout.push(data);
  ...
  it('Scenario: Given recursive dry-run publish with a changed-since filter, When confirmed, Then packages with changed files are packed', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'pnpm-workspace.yaml'), 'packages:\n  - packages/*\n', 'utf8');
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9', private: true }), 'utf8');
    await fsp.writeFile(path.join(packageADir, 'package.json'), JSON.stringify({ name: 'changed-a', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(path.join(packageADir, 'index.js'), 'module.exports = 1;\n', 'utf8');
    await fsp.writeFile(path.join(packageBDir, 'package.json'), JSON.stringify({ name: 'changed-b', version: '1.0.0' }), 'utf8');
  ...
  it('Scenario: Given recursive dry-run publish with changed file ignore patterns, When confirmed, Then ignored file-only packages are not packed', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'pnpm-workspace.yaml'), 'packages:\n  - packages/*\n', 'utf8');
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9', private: true }), 'utf8');
    await fsp.writeFile(path.join(packageADir, 'package.json'), JSON.stringify({ name: 'ignore-changed-a', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(path.join(packageADir, 'README.md'), 'old\n', 'utf8');
    await fsp.writeFile(path.join(packageBDir, 'package.json'), JSON.stringify({ name: 'ignore-changed-b', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(path.join(packageBDir, 'index.js'), 'module.exports = 1;\n', 'utf8');
    await execFileAsync('git', ['init', '-q'], { cwd: workspaceRoot });
    await execFileAsync('git', ['config', 'user.email', 'test@example.com'], { cwd: workspaceRoot });
    await execFileAsync('git', ['config', 'user.name', 'Test'], { cwd: workspaceRoot });
    ...
    await scheduler.intercept(
      {
        command: 'publish',
        cwd: workspaceRoot,
        args: ['--recursive', '--dry-run', '--filter', '[HEAD]', '--changed-files-ignore-pattern', '**/README.md', '--no-git-checks'],
      },
      {
        log: (stream, data) => {
          if (stream === 'stdout') stdout.push(data);
  ...
  it('Scenario: Given recursive dry-run publish with a package changed-since filter, When confirmed, Then changed package identity is required', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'pnpm-workspace.yaml'), 'packages:\n  - packages/*\n', 'utf8');
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9', private: true }), 'utf8');
    await fsp.writeFile(path.join(packageADir, 'package.json'), JSON.stringify({ name: 'changed-suffix-a', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(path.join(packageADir, 'index.js'), 'module.exports = 1;\n', 'utf8');
    await fsp.writeFile(path.join(packageBDir, 'package.json'), JSON.stringify({ name: 'changed-suffix-b', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(path.join(packageBDir, 'index.js'), 'module.exports = 1;\n', 'utf8');
    await execFileAsync('git', ['init', '-q'], { cwd: workspaceRoot });
    await execFileAsync('git', ['config', 'user.email', 'test@example.com'], { cwd: workspaceRoot });
    await execFileAsync('git', ['config', 'user.name', 'Test'], { cwd: workspaceRoot });
  ...
  it('Scenario: Given recursive dry-run publish with a directory changed-since filter, When confirmed, Then changed package path is required', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'pnpm-workspace.yaml'), 'packages:\n  - packages/*\n', 'utf8');
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9', private: true }), 'utf8');
    await fsp.writeFile(path.join(packageADir, 'package.json'), JSON.stringify({ name: 'changed-dir-a', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(path.join(packageADir, 'index.js'), 'module.exports = 1;\n', 'utf8');
    await fsp.writeFile(path.join(packageBDir, 'package.json'), JSON.stringify({ name: 'changed-dir-b', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(path.join(packageBDir, 'index.js'), 'module.exports = 1;\n', 'utf8');
    await execFileAsync('git', ['init', '-q'], { cwd: workspaceRoot });
    await execFileAsync('git', ['config', 'user.email', 'test@example.com'], { cwd: workspaceRoot });
    await execFileAsync('git', ['config', 'user.name', 'Test'], { cwd: workspaceRoot });
  ...
  it('Scenario: Given recursive dry-run publish with a changed-since dependent graph filter, When confirmed, Then dependents are packed after changed packages', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'pnpm-workspace.yaml'), 'packages:\n  - packages/*\n', 'utf8');
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9', private: true }), 'utf8');
    await fsp.writeFile(path.join(depDir, 'package.json'), JSON.stringify({ name: 'changed-dep', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(path.join(depDir, 'index.js'), 'module.exports = 1;\n', 'utf8');
    await fsp.writeFile(
      path.join(appDir, 'package.json'),
      JSON.stringify({ name: 'changed-app', version: '1.0.0', dependencies: { 'changed-dep': 'workspace:*' } }),
      'utf8',
    );
    await fsp.writeFile(path.join(appDir, 'index.js'), 'module.exports = 1;\n', 'utf8');
    await fsp.writeFile(path.join(otherDir, 'package.json'), JSON.stringify({ name: 'changed-other', version: '1.0.0' }), 'utf8');
  ...
  it('Scenario: Given recursive dry-run publish with no matching filters, When confirmed, Then it exits as a native no-op', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'pnpm-workspace.yaml'), 'packages:\n  - packages/*\n', 'utf8');
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9', private: true }), 'utf8');
    await fsp.writeFile(path.join(packageDir, 'package.json'), JSON.stringify({ name: 'available-pkg', version: '1.2.3' }), 'utf8');
    const exit = vi.fn();
    const stdout: string[] = [];
    const stderr: string[] = [];
    ...
    await scheduler.intercept(
      {
        command: 'publish',
        cwd: workspaceRoot,
        args: ['--recursive', '--dry-run', '--report-summary', '--filter', 'missing-pkg', '--no-git-checks'],
      },
      {
        log: (stream, data) => {
          if (stream === 'stdout') stdout.push(data);
    ...
    expect(readPackageTarballMock).not.toHaveBeenCalled();
    expect(publishPackageMock).not.toHaveBeenCalled();
    await expect(fsp.access(path.join(workspaceRoot, 'pnpm-publish-summary.json'))).rejects.toThrow();
    expect(exit).toHaveBeenCalledWith(0);
  });
  ...
  it('Scenario: Given recursive dry-run publish with unsupported bare graph filters, When confirmed, Then it fails before packing', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'pnpm-workspace.yaml'), 'packages:\n  - packages/*\n', 'utf8');
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9', private: true }), 'utf8');
    await fsp.writeFile(path.join(packageDir, 'package.json'), JSON.stringify({ name: 'unsupported-graph-pkg', version: '1.0.0' }), 'utf8');
    ...
    for (const option of ['--filter', '--filter-prod']) {
      for (const filter of ['...', '^...', '...^', '......', '...^...']) {
        ...
        expect(readPackageTarballMock).not.toHaveBeenCalled();
        expect(publishPackageMock).not.toHaveBeenCalled();
        expect(exit).toHaveBeenCalledWith(1, expect.stringContaining('Unsupported package selector'));
      }
    }
  });
  ...
  it('Scenario: Given recursive dry-run publish with fail-if-no-match, When no filters match, Then it exits as a native failure', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'pnpm-workspace.yaml'), 'packages:\n  - packages/*\n', 'utf8');
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9', private: true }), 'utf8');
    await fsp.writeFile(path.join(packageDir, 'package.json'), JSON.stringify({ name: 'native-no-match-pkg', version: '1.0.0' }), 'utf8');
    const exit = vi.fn();
    const stdout: string[] = [];
    const stderr: string[] = [];
    ...
    await scheduler.intercept(
      {
        command: 'publish',
        cwd: workspaceRoot,
        args: ['--recursive', '--dry-run', '--filter', 'missing-pkg', '--fail-if-no-match', '--no-git-checks'],
      },
      {
        log: (stream, data) => {
          if (stream === 'stdout') stdout.push(data);
  ...
  it('Scenario: Given recursive dry-run publish with a dependency graph filter, When confirmed, Then dependencies are packed before the selected package', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'pnpm-workspace.yaml'), 'packages:\n  - packages/*\n', 'utf8');
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9', private: true }), 'utf8');
    await fsp.writeFile(path.join(leafDir, 'package.json'), JSON.stringify({ name: 'graph-leaf', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(
      path.join(midDir, 'package.json'),
      JSON.stringify({ name: 'graph-mid', version: '1.0.0', dependencies: { 'graph-leaf': 'workspace:*' } }),
      'utf8',
    );
    await fsp.writeFile(
      path.join(appDir, 'package.json'),
      JSON.stringify({ name: 'graph-app', version: '1.0.0', dependencies: { 'graph-mid': 'workspace:*' } }),
      'utf8',
    );
    const exit = vi.fn();
    const stdout: string[] = [];
  ...
  it('Scenario: Given recursive dry-run publish with a combined graph filter, When confirmed, Then dependencies and dependents are packed around the selected package', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'pnpm-workspace.yaml'), 'packages:\n  - packages/*\n', 'utf8');
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9', private: true }), 'utf8');
    await fsp.writeFile(path.join(leafDir, 'package.json'), JSON.stringify({ name: 'both-leaf', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(
      path.join(midDir, 'package.json'),
      JSON.stringify({ name: 'both-mid', version: '1.0.0', dependencies: { 'both-leaf': 'workspace:*' } }),
      'utf8',
    );
    await fsp.writeFile(
      path.join(appDir, 'package.json'),
      JSON.stringify({ name: 'both-app', version: '1.0.0', dependencies: { 'both-mid': 'workspace:*' } }),
      'utf8',
    );
    await fsp.writeFile(path.join(otherDir, 'package.json'), JSON.stringify({ name: 'both-other', version: '1.0.0' }), 'utf8');
    const exit = vi.fn();
    const stdout: string[] = [];
  ...
  it('Scenario: Given recursive dry-run publish with combined graph caret filters, When confirmed, Then native seed matching is preserved', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'pnpm-workspace.yaml'), 'packages:\n  - packages/*\n', 'utf8');
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9', private: true }), 'utf8');
    await fsp.writeFile(path.join(leafDir, 'package.json'), JSON.stringify({ name: 'caret-leaf', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(
      path.join(midDir, 'package.json'),
      JSON.stringify({ name: 'caret-mid', version: '1.0.0', dependencies: { 'caret-leaf': 'workspace:*' } }),
      'utf8',
    );
    await fsp.writeFile(
      path.join(appDir, 'package.json'),
      JSON.stringify({ name: 'caret-app', version: '1.0.0', dependencies: { 'caret-mid': 'workspace:*' } }),
      'utf8',
    );
    await fsp.writeFile(path.join(otherDir, 'package.json'), JSON.stringify({ name: 'caret-other', version: '1.0.0' }), 'utf8');
  ...
  it('Scenario: Given recursive dry-run publish from a package with current-project graph filters, When confirmed, Then only the current package is packed', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'pnpm-workspace.yaml'), 'packages:\n  - packages/*\n', 'utf8');
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9', private: true }), 'utf8');
    await fsp.writeFile(path.join(leafDir, 'package.json'), JSON.stringify({ name: 'dot-graph-leaf', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(
      path.join(appDir, 'package.json'),
      JSON.stringify({ name: 'dot-graph-app', version: '1.0.0', dependencies: { 'dot-graph-leaf': 'workspace:*' } }),
      'utf8',
    );
    await fsp.writeFile(
      path.join(consumerDir, 'package.json'),
      JSON.stringify({ name: 'dot-graph-consumer', version: '1.0.0', dependencies: { 'dot-graph-app': 'workspace:*' } }),
      'utf8',
    );
    await fsp.writeFile(path.join(otherDir, 'package.json'), JSON.stringify({ name: 'dot-graph-other', version: '1.0.0' }), 'utf8');
  ...
  it('Scenario: Given recursive dry-run publish with a production dependency graph filter, When confirmed, Then dev dependencies are not packed', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'pnpm-workspace.yaml'), 'packages:\n  - packages/*\n', 'utf8');
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9', private: true }), 'utf8');
    await fsp.writeFile(path.join(prodDir, 'package.json'), JSON.stringify({ name: 'filter-prod-prod', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(path.join(optionalDir, 'package.json'), JSON.stringify({ name: 'filter-prod-optional', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(path.join(peerDir, 'package.json'), JSON.stringify({ name: 'filter-prod-peer', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(path.join(devDir, 'package.json'), JSON.stringify({ name: 'filter-prod-dev', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(
      path.join(appDir, 'package.json'),
      JSON.stringify({
    ...
    await scheduler.intercept(
      {
        command: 'publish',
        cwd: workspaceRoot,
        args: ['--recursive', '--dry-run', '--filter-prod', 'filter-prod-app...', '--no-git-checks'],
      },
      {
        log: (stream, data) => {
          if (stream === 'stdout') stdout.push(data);
    ...
    await scheduler.intercept(
      {
        command: 'publish',
        cwd: workspaceRoot,
        args: ['--recursive', '--dry-run', '--filter-prod', '...filter-prod-dev', '--no-git-checks'],
      },
      {
        log: (stream, data) => {
          if (stream === 'stdout') stdout.push(data);
  ...
  it('Scenario: Given recursive dry-run publish with a dependent graph filter, When confirmed, Then dependents are packed after the selected package', async () => {
    ...
    await fsp.writeFile(
      path.join(appDir, 'package.json'),
      JSON.stringify({ name: 'graph-app', version: '1.0.0', devDependencies: { 'graph-mid': 'workspace:*' } }),
      'utf8',
    );
    const exit = vi.fn();
    const stdout: string[] = [];
  ...
  it('Scenario: Given recursive dry-run publish with a package-name glob filter, When confirmed, Then matching package names are packed', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'pnpm-workspace.yaml'), 'packages:\n  - packages/*\n', 'utf8');
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9', private: true }), 'utf8');
    await fsp.writeFile(path.join(packageADir, 'package.json'), JSON.stringify({ name: 'native-recursive-a', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(path.join(packageBDir, 'package.json'), JSON.stringify({ name: 'native-recursive-b', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(path.join(otherDir, 'package.json'), JSON.stringify({ name: 'other-pkg', version: '2.0.0' }), 'utf8');
    const exit = vi.fn();
    const log = vi.fn();
  ...
  it('Scenario: Given recursive dry-run publish with a scoped package glob filter, When confirmed, Then matching scoped names are packed', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'pnpm-workspace.yaml'), 'packages:\n  - packages/*\n', 'utf8');
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9', private: true }), 'utf8');
    await fsp.writeFile(path.join(packageDir, 'package.json'), JSON.stringify({ name: '@scope/a', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(path.join(otherDir, 'package.json'), JSON.stringify({ name: '@other/b', version: '1.0.0' }), 'utf8');
    const exit = vi.fn();
    const log = vi.fn();
  ...
  it('Scenario: Given recursive dry-run publish with only a negated filter, When confirmed, Then excluded packages are not packed', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'pnpm-workspace.yaml'), 'packages:\n  - packages/*\n', 'utf8');
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9', private: true }), 'utf8');
    await fsp.writeFile(path.join(packageADir, 'package.json'), JSON.stringify({ name: 'native-neg-a', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(path.join(packageBDir, 'package.json'), JSON.stringify({ name: 'native-neg-b', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(path.join(packageCDir, 'package.json'), JSON.stringify({ name: 'other-c', version: '1.0.0' }), 'utf8');
    const exit = vi.fn();
    ...
    await scheduler.intercept(
      {
        command: 'publish',
        cwd: workspaceRoot,
        args: ['--recursive', '--dry-run', '--filter', 'native-neg-*', '--filter', '!native-neg-b', '--no-git-checks'],
      },
      { log: () => {}, exit },
    );
    const pending = store.getEvents().find((event) => event.status === 'pending');
  ...
  it('Scenario: Given recursive dry-run publish with --json, When confirmed, Then stdout uses native success lines', async () => {
    ...
    await fsp.writeFile(path.join(workspaceRoot, 'pnpm-workspace.yaml'), 'packages:\n  - packages/*\n', 'utf8');
    await fsp.writeFile(path.join(workspaceRoot, 'package.json'), JSON.stringify({ name: 'workspace-root', version: '9.9.9', private: true }), 'utf8');
    await fsp.writeFile(path.join(packageADir, 'package.json'), JSON.stringify({ name: 'native-json-a', version: '1.0.0' }), 'utf8');
    await fsp.writeFile(path.join(packageBDir, 'package.json'), JSON.stringify({ name: 'native-json-b', version: '1.0.0' }), 'utf8');
    const exit = vi.fn();
    const stdout: string[] = [];
    const stderr: string[] = [];
  ...
  it('Scenario: Given a dry-run publish, When confirmed, Then it packs without credentials or registry writes', async () => {
    ...
    expect(noticeText).toContain('npm notice Publishing to http://registry.test/ with tag latest and default access (dry-run)\n');
    expect(summarizePackageTarballMock).toHaveBeenCalledWith(Buffer.from('placeholder tarball'));
    expect(exit).toHaveBeenCalledWith(0);
  });
  ...
  it('Scenario: Given a dry-run publish with --json, When confirmed, Then stdout is parseable package JSON', async () => {
    ...
    expect(outputText).not.toContain('packing');
    expect(stderr.join('')).toBe('npm notice Publishing to http://registry.test/ with tag latest and default access (dry-run)\n');
    expect(packPackageMock).toHaveBeenCalledWith(packageDir);
    expect(publishPackageMock).not.toHaveBeenCalled();
    expect(exit).toHaveBeenCalledWith(0);
  });
  ...
  it('Scenario: Given a CLI publish cwd inside a workspace, When intercepted, Then the daemon records the workspace root', async () => {
    ...
    expect(store.getWorkspaces()).toEqual([{ path: workspaceRoot, pinned: false, addedAt: expect.any(Number) }]);
    const pending = store.getEvents().find((e) => e.status === 'pending');
    expect(pending?.payload?.kind).toBe('publish');
    if (pending?.payload?.kind !== 'publish') return;
    expect(pending.payload.data.source).toEqual({ kind: 'directory', path: packageDir });
