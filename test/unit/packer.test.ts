...
import { normalizeOutputChunk, packPackage, readPackageTarball, summarizePackageTarball } from '../../src/daemon/packer.js';
...
describe('Feature: packer source-directory conservation', () => {
  it('Scenario: Given package-local tarballs already exist, When packing, Then user artifacts are preserved', async () => {
    ...
    await expect(fsp.access(path.join(packageDir, 'preserve-tarballs-1.0.0.tgz'))).rejects.toThrow();
  });
...
describe('Feature: packer tarball source conservation', () => {
  ...
  it('Scenario: Given a packed npm package, When summarizing it for publish JSON, Then native file-list facts are projected from the tarball', async () => {
    ...
    expect(summary.files).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ path: 'package.json', size: expect.any(Number), mode: expect.any(Number) }),
        expect.objectContaining({ path: 'index.js', size: expect.any(Number), mode: expect.any(Number) }),
      ]),
    );
  });
  ...
  it('Scenario: Given a packed package with bundled dependencies, When summarizing it, Then bundled files stay out of own package contents', async () => {
    ...
    await fsp.writeFile(path.join(packageDir, 'pnpm-workspace.yaml'), 'nodeLinker: hoisted\n', 'utf8');
    await fsp.writeFile(
      path.join(packageDir, 'package.json'),
      JSON.stringify({
    ...
    await fsp.writeFile(path.join(dependencyDir, 'package.json'), JSON.stringify({ name: 'local-dep', version: '1.0.0' }), 'utf8');
    ...
    expect(summary.unpackedSize).toBeGreaterThan(summary.files.reduce((total, file) => total + file.size, 0));
    expect(summary.files).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ path: 'package.json', size: expect.any(Number), mode: expect.any(Number) }),
      ]),
