...
import { openEventDb, insertEvent, updateEvent, queryEvents, recentEvents } from '../../src/daemon/event-db.js';
...
describe('event-db persistence', () => {
  it('inserts and reads back an event with full fidelity', () => {
    const evt = makeEvent({
      payload: { kind: 'publish', data: { source: { kind: 'directory', path: '/p' }, args: [], target: { name: '@scope/pkg', version: '1.0.0', path: '/p' } } },
      result: 'Published',
      clockDriftRecovered: true,
      groupId: 'grp-1',
      tarballSummary: { files: [{ path: 'package/index.js', size: 42, mode: 420 }], unpackedSize: 42, entryCount: 1, bundled: [] },
    });
    insertEvent(db, evt);
    const got = recentEvents(db, 10);
    expect(got).toHaveLength(1);
    expect(got[0]).toMatchObject({ id: evt.id, kind: 'publish', status: 'success', profile: 'alice' });
  ...
  it('serializes undefined optionals as absent (not null) on readback', () => {
    const evt = makeEvent({ profileOverride: undefined, resolvedAt: undefined, clockDriftRecovered: undefined });
    insertEvent(db, evt);
  ...
  it('queryEvents filters by package name (payload match)', () => {
    insertEvent(db, makeEvent({ id: 'a', payload: { kind: 'publish', data: { source: { kind: 'directory', path: '/p' }, args: [], target: { name: '@scope/pkg', version: '1.0.0', path: '/p' } } } }));
    insertEvent(db, makeEvent({ id: 'b', payload: { kind: 'publish', data: { source: { kind: 'directory', path: '/p' }, args: [], target: { name: 'other-lib', version: '2.0.0', path: '/p' } } } }));
  ...
  it('queryEvents filters by keyword (AND match on kind/status/result)', () => {
    ...
    const r = queryEvents(db, { status: 'history', keywords: ['failed', 'otp'], page: 0, limit: 10 });
    expect(r.total).toBe(1);
    expect(r.rows[0]!.id).toBe('a');
  });
