...
} from "../../webui/src/lib/qr-scanner-policy.js";
...
describe("Feature: WebUI QR scanner capture policy", () => {
  ...
  it("Scenario: Given a rotated camera QR, When browser detection is available, Then native detection remains part of the decoder strategy", () => {
    expect(QR_DECODER_CONFIG.useBarCodeDetectorIfSupported).toBe(true);
  });

  it("Scenario: Given a camera with its own aspect ratio, When decoding begins, Then the decoder receives its natural frame geometry", () => {
    expect("aspectRatio" in QR_VIDEO_CONSTRAINTS).toBe(false);
  });
});
