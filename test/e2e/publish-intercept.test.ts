...
async function writeFixturePackage(name: string, version: string, dir: string = pkgDir): Promise<void> {
  ...
  await fsp.writeFile(path.join(dir, 'package.json'), JSON.stringify({ name, version, description: 'e2e fixture' }));
...
beforeAll(async () => {
  ...
  const url = process.env.PNPM_PUB_E2E_REGISTRY ?? (await verdaccioReachable() ? 'http://localhost:4873' : registryUrl);
  ...
  if (handles) {
    ...
    handles.store.setCredentials('e2e-author', { token: 'mock-token', totpSecret: 'JBSWY3DPEHPK3PXP' });
