...
async function waitForServer(url: string): Promise<void> {
	...
	throw new Error(`Timed out waiting for WebUI dev server. Last error: ${String(lastError)}\n${serverLog}`);
}
...
beforeAll(async () => {
	...
	server = spawn(
		'pnpm',
		['--dir', 'webui', 'exec', 'vite', 'dev', '--host', '127.0.0.1', '--port', String(port), '--strictPort'],
		{
...
describe('Feature: Renew route rendered projection', () => {
	it('Scenario: Given renew route reasons, When a browser opens the routes, Then only expired routes say Renew Token', async () => {
		...
		await expect(titleFor('/renew?reason=action-required')).resolves.toBe('Re-apply Credentials · pnpm-pub');
	...
	it('Scenario: Given daemon renew failure without explicit error, When submitted in a browser, Then fallback error follows the route source', async () => {
		...
		await expect(defaultErrorFor('/renew?reason=action-required')).resolves.toBe('Credential re-apply failed.');
